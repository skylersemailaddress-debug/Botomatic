...
function buildWaveCatalog(hints: Set<string>): WaveDef[] {
  const catalog: WaveDef[] = [
    ...
    {
      id: "truth_memory",
      name: "Truth + Memory Engine",
      description: "Build the durable cognitive substrate: truth objects, memory classes, contradiction detection.",
      ...
      packets: [
        { goal: "Implement truth object schema with confidence, status, and contradiction refs", scope: ["packages/truth-engine/src/"], risk: "medium" },
        { goal: "Implement 9 memory classes with durable storage", scope: ["packages/memory-engine/src/"], risk: "high" },
        { goal: "Prove memory influences decisions — not just retrieval", scope: ["packages/memory-engine/src/"], risk: "high" },
      ],
    },
    {
      id: "intelligence_shell",
      name: "Synthetic Intelligence Shell",
      description: "Build the Nexus primary UI: command spine, operational focus pane, and summoned deep surfaces.",
      ...
      acceptance: ["command spine accepts NL + slash commands", "focus pane shows real state", "deep surfaces summon correctly"],
      packets: [
        { goal: "Implement command spine with NL, slash, correction, and approval input lanes", scope: ["apps/control-plane/src/components/CommandSpine"], risk: "medium" },
        { goal: "Implement operational focus pane showing current state, changes, interventions", scope: ["apps/control-plane/src/components/FocusPane"], risk: "high" },
        { goal: "Implement summoned deep surfaces (World, Truth, Predictions, Execution, Builder Mode)", scope: ["apps/control-plane/src/surfaces/"], risk: "high" },
      ],
    },
    {
...
export function compileSpecToMission(input: CompilerInput): CompilerOutput {
  ...
  return { mission, sourceHash, specHash, waveCount: waves.length };
}
