...
export function applyUIEditCommand(document: EditableUIDocument, command: UIEditCommand, context: UIEditMutationContext = {}): UIEditMutationResult {
  const inputValid = validateEditableUIDocument(document); if (!inputValid.valid) return { status: "invalid", success: false, issues: inputValid.issues.map((m) => ({ code: "invalid_document", message: m })), beforeDocumentId: document.id, beforeVersion: document.version, changedNodeIds: [], changedPageIds: [], previewPatch: createLivePreviewPatch([{ kind: "validationFailed" }], context.now), claimBoundary: CLAIM };
  const resolution = resolveUIEditTarget(command, document, context);
  if ((command.safety.requiresResolution || resolution.requiresResolution) && resolution.status !== "resolved" && command.kind !== "addPage" && command.kind !== "retheme") return { status: "needsResolution", success: false, issues: [{ code: "needs_resolution", message: "Target requires resolution" }], beforeDocumentId: document.id, beforeVersion: document.version, changedNodeIds: [], changedPageIds: [], previewPatch: createLivePreviewPatch([], context.now), claimBoundary: CLAIM };
  if ((command.safety.requiresConfirmation || HIGH.has(command.kind)) && context.confirmed !== true) return { status: "blocked", success: false, issues: [{ code: "confirmation_required", message: "Command requires confirmation" }], beforeDocumentId: document.id, beforeVersion: document.version, changedNodeIds: [], changedPageIds: [], previewPatch: createLivePreviewPatch([], context.now), claimBoundary: CLAIM };
  return applyResolvedUIEditCommand(document, command, resolution, context);
}
