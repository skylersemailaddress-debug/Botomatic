...
import { UI_EXPORT_DEPLOY_CAVEAT, type UIDeployEnvironmentVariable, type UIExportDeployPlan } from "./uiExportDeployModel";

type Input = {
  fullProjectGenerationPlan?: any;
  sourcePatchPlan?: any;
  sourceFiles?: Record<string, string>;
  scalabilityPlan?: any;
  reliabilityRepairPlan?: any;
  uxControlPlan?: any;
  environmentVariables?: UIDeployEnvironmentVariable[];
  options: { provider: any; framework: any; targetName: string; allowLiveDeploy?: boolean; requireRollbackPlan?: boolean; target?: { staticExport?: boolean; hasApiRoutes?: boolean; rollbackReference?: string } };
};

export function createUIExportDeployPlan(input: Input): UIExportDeployPlan {
  ...
  const targetPlan = createUIDeployTargetPlan({ provider: input.options.provider, framework: input.options.framework, bundleManifest, environmentVariables: input.environmentVariables ?? [], target, scalabilityPlan: input.scalabilityPlan, reliabilityRepairPlan: input.reliabilityRepairPlan, uxControlPlan: input.uxControlPlan, rollbackPlanRequired: Boolean(input.options.requireRollbackPlan), rollbackReference: input.options.target?.rollbackReference });
  ...
  if (input.options.requireRollbackPlan && !input.options.target?.rollbackReference) blockedReasons.push("rollback required but missing");
