...
export function createUISourcePatchFromSyncPlan(plan: UISourceSyncPlan, mapping: UISourceFileMapping): UISourcePatch {
  ...
  for (const op of plan.operations) {
    if (op.kind === "manualReviewRequired") { operations.push({ kind: "manualReviewRequired", targetFilePath: op.targetFileHint ?? "manual-review", pageIds: [...op.pageIds], nodeIds: [...op.nodeIds], reason: op.reason ?? "ambiguous mapping", confidence: "low", requiresManualReview: true, sourceKind: "unknown" }); continue; }
    ...
    if (!mapped?.filePath) { operations.push({ kind: "manualReviewRequired", targetFilePath: "manual-review", pageIds: [...op.pageIds], nodeIds: [...op.nodeIds], reason: "mapping missing", confidence: "low", requiresManualReview: true, sourceKind: "unknown" }); continue; }
    operations.push({ kind: "replaceText", targetFilePath: mapped.filePath, pageIds: [...op.pageIds], nodeIds: [...op.nodeIds], beforeSnippet: "// before", afterSnippet: "// after", confidence: "medium", requiresManualReview: false, sourceKind: kindFromFile(mapped.filePath) });
  }
