...
const writes:string[]=[];
const fsAdapter={readFile:()=>"old",writeFile:(p:string)=>{writes.push(p);},exists:()=>true};
...
const confirmed=applyUISourcePatch(good,fsAdapter,{mode:"confirmedApply",projectRoot:".",confirmationMarker:true}); assert.ok(confirmed.ok);
