...
assert.equal(validateEditableUIDocument(doc).valid, true);
...
assert.notEqual(cloned.pages[0], doc.pages[0]);
...
assert.equal(validateEditableUIDocument(invalid).valid, false);

assert.equal(JSON.stringify(blueprint), inputSnapshot);
