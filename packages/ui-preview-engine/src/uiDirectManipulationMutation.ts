...
import { cloneEditableUIDocument, createStableNodeId, type EditableUIDocument } from "./uiDocumentModel";
...
const rootBlocked = new Set(["pageRoot"]);

export function applyUIDirectManipulation(document: EditableUIDocument, action: UIDirectManipulationAction): UIDirectManipulationResult {
  ...
  if (action.type === "selectNode") return done(document, next, [], [], [], `Selected ${action.nodeId}`, action.nodeId);

  if (action.type === "deleteNode" && action.confirm !== true) return needsConfirm(document, "Delete requires confirmation");
  if (action.type === "deleteNode") {
    if (rootBlocked.has(node.kind) || page.rootNodeIds.includes(node.id)) return blocked(document, "Cannot delete page root node");
    if (node.parentId) page.nodes[node.parentId].childIds = page.nodes[node.parentId].childIds.filter((id) => id !== node.id);
    delete page.nodes[node.id];
    return finalize(document, next, [node.id], `Deleted node ${node.id}`);
  }
  ...
  if (action.type === "reorderNode") {
    ...
    const idx = Math.min(action.targetIndex, parent.childIds.length);
  ...
  if (action.type === "duplicateNode") {
    ...
    const copy = { ...node, id: newId, childIds: [], props: { ...node.props }, style: { ...node.style }, layout: { ...node.layout }, identity: { ...node.identity, nodeId: newId, updatedAt: new Date().toISOString() } };
...
function finalize(before: EditableUIDocument, after: EditableUIDocument, changedNodeIds: string[], summary: string): UIDirectManipulationResult {
  ...
  return done(before, after, changedNodeIds, diff.operations, createLivePreviewPatch(diff.operations as never).operations, `${summary}; ${summarizeUIDocumentDiff(diff)}`);
}
function done(before: EditableUIDocument, next: EditableUIDocument, changedNodeIds: string[], diffOps: Array<{ kind: string; nodeId?: string; pageId?: string }>, previewOps: Array<{ kind: string; nodeId?: string; pageId?: string }>, summary: string, selectedNodeId?: string): UIDirectManipulationResult {
...
function needsConfirm(document: EditableUIDocument, message: string): UIDirectManipulationResult { return { status: "needsConfirmation", nextDocument: document, changedNodeIds: [], diff: { operations: [] }, previewPatch: { operations: [] }, summary: message }; }
