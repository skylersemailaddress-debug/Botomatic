...
function testJobEnqueueIncludesTenantScope() {
  ...
  // All four idempotentEnqueueJob call sites must pass owner_user_id and tenant_id
  const enqueueCallSites = [...serverApp.matchAll(/idempotentEnqueueJob\(\{[^}]+\}\)/gs)];
  assert.ok(
    enqueueCallSites.length >= 4,
    `Expected at least 4 idempotentEnqueueJob call sites, found ${enqueueCallSites.length}`,
  );

  for (const [callsite] of enqueueCallSites) {
    assert.ok(
      callsite.includes("owner_user_id"),
      `Every idempotentEnqueueJob call must include owner_user_id: ${callsite.slice(0, 120)}`,
    );
    assert.ok(
      callsite.includes("tenant_id"),
      `Every idempotentEnqueueJob call must include tenant_id: ${callsite.slice(0, 120)}`,
    );
  }

  // The raw enqueueJob call site (repair/replay) must also include tenant scope
  const rawEnqueue = serverApp.slice(serverApp.indexOf("/repair/replay"), serverApp.indexOf("/repair/replay") + 600);
  if (rawEnqueue.includes("enqueueJob(")) {
    assert.ok(
      rawEnqueue.includes("owner_user_id"),
      "repair/replay enqueueJob call must include owner_user_id",
    );
    assert.ok(
      rawEnqueue.includes("tenant_id"),
      "repair/replay enqueueJob call must include tenant_id",
    );
  }

  console.log(`  ✓ all ${enqueueCallSites.length} job enqueue call sites include owner_user_id and tenant_id`);
}
