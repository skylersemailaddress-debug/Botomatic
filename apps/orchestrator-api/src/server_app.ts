...
function appendRunLog(project: StoredProjectRecord, packetId: string, level: "info" | "warn" | "error", event: string, details?: Record<string, unknown>) {
  const runs = { ...((project.runs || {}) as Record<string, any>) };
  const existing = runs[packetId] || { projectId: project.projectId, packetId, status: "queued", retryCount: 0, logs: [] };
  runs[packetId] = {
    ...existing,
    logs: [...(existing.logs || []), { timestamp: now(), level, event, details }],
  };
  project.runs = runs;
}
...
async function processJob(config: RuntimeConfig, job: QueueJobRecord) {
  ...
  appendRunLog(project, packet.packetId, "info", "packet_executing");
  await persistProject(config, project);
  ...
  try {
    ...
    if (!result.success) {
      appendRunLog(project, packet.packetId, "error", "executor_failed", { summary: result.summary || "Executor failed" });
    ...
    appendRunLog(project, packet.packetId, "info", "validation_started");
    ...
    if (validation.status !== "passed") {
      appendRunLog(project, packet.packetId, "error", "validation_failed", { summary: validation.summary, checks: validation.checkResults || [] });
      const failedState = markPacketFailed({ projectId: project.projectId, status: project.status as any, packets: getPackets(project), runs: project.runs as any }, packet.packetId);
      project.plan = { ...((project.plan as any) || {}), packets: failedState.packets };
      project.runs = failedState.runs || project.runs;
      recomputeProjectStatus(project);
    ...
    appendRunLog(project, packet.packetId, "info", "validation_passed", { checks: validation.checkResults || [] });
    ...
    appendRunLog(project, packet.packetId, "error", "job_failed", { message: String(error?.message || error) });
    const failedState = markPacketFailed({ projectId: project.projectId, status: project.status as any, packets: getPackets(project), runs: project.runs as any }, packet.packetId);
    project.plan = { ...((project.plan as any) || {}), packets: failedState.packets };
    project.runs = failedState.runs || project.runs;
    recomputeProjectStatus(project);
