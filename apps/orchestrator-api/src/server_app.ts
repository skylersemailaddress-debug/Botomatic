...
import { classifyGitFailure, isRecoverableGitFailure } from "../../../packages/github-adapter/src/failurePolicy";
...
type RequestActor = { actorId: string; actorSource: "x-actor-id" | "x-user-email" | "bearer_token" | "anonymous"; };
type QueueJobRecord = { job_id: string; project_id: string; packet_id: string; status: "queued" | "running" | "succeeded" | "failed"; attempts: number; };
type ChangedFile = { path?: string; body?: string; };
...
function toStored(record: { projectId: string; name: string; request: string; status: string; masterTruth?: any; plan?: any; runs?: any; validations?: any; gitOperations?: Record<string, any>; gitResults?: Record<string, any>; }): StoredProjectRecord {
  const timestamp = now();
  return { projectId: record.projectId, name: record.name, request: record.request, status: record.status, masterTruth: record.masterTruth ?? null, plan: record.plan ?? null, runs: record.runs ?? null, validations: record.validations ?? null, gitOperations: record.gitOperations ?? null, gitResults: record.gitResults ?? null, createdAt: timestamp, updatedAt: timestamp };
}

function getExecutor() {
  if (process.env.EXECUTOR === "claude") return new ClaudeCodeExecutor({ baseUrl: process.env.CLAUDE_EXECUTOR_URL!, apiKey: process.env.CLAUDE_EXECUTOR_KEY });
  return MockExecutor;
}

function getGitHub() { return new GitHubRuntime({ token: process.env.GITHUB_TOKEN!, owner: "skylersemailaddress-debug", repo: "Botomatic" }); }

function getRequestActor(req: express.Request, config: RuntimeConfig): RequestActor {
  const actorIdHeader = req.header("x-actor-id");
  if (actorIdHeader) return { actorId: actorIdHeader, actorSource: "x-actor-id" };
  const userEmailHeader = req.header("x-user-email");
  if (userEmailHeader) return { actorId: userEmailHeader, actorSource: "x-user-email" };
  if (config.auth.enabled && config.auth.token) return { actorId: `api_token:${String(config.auth.token).slice(0, 6)}`, actorSource: "bearer_token" };
  return { actorId: "anonymous", actorSource: "anonymous" };
}

function requireApiAuth(config: RuntimeConfig): express.RequestHandler {
  return (req, res, next) => {
    if (!config.auth.enabled || !config.auth.token) return res.status(500).json({ error: "API auth is not configured", authEnabled: config.auth.enabled, authImplementation: config.auth.implementation });
    const authorization = req.header("authorization") || "";
    const expected = `Bearer ${config.auth.token}`;
    if (authorization !== expected) return res.status(401).json({ error: "Unauthorized", authEnabled: config.auth.enabled, authImplementation: config.auth.implementation });
    return next();
  };
}
...
function handleRouteError(res: express.Response, config: RuntimeConfig, error: unknown, route: string, actor?: RequestActor) {
  ...
  console.error(JSON.stringify({ event: "route_error", route, repositoryMode: config.repository.mode, repositoryImplementation: config.repository.implementation, authEnabled: config.auth.enabled, authImplementation: config.auth.implementation, actorId: actor?.actorId || "unknown", actorSource: actor?.actorSource || "unknown", message, workerId, timestamp: now() }));
  return res.status(500).json({ error: message, repositoryMode: config.repository.mode, repositoryImplementation: config.repository.implementation, authEnabled: config.auth.enabled, authImplementation: config.auth.implementation, workerId });
}

function getPackets(project: StoredProjectRecord): any[] { const plan = (project.plan || {}) as any; return Array.isArray(plan.packets) ? plan.packets : []; }
function recomputeProjectStatus(project: StoredProjectRecord) {
  const packets = getPackets(project);
  if (packets.length === 0) { project.status = "queued"; return; }
  if (packets.every((packet: any) => packet.status === "complete")) { project.status = "complete"; return; }
  if (packets.some((packet: any) => packet.status === "executing")) { project.status = "executing"; return; }
  if (packets.some((packet: any) => packet.status === "pending")) { project.status = "queued"; return; }
  if (packets.some((packet: any) => packet.status === "blocked")) { project.status = "blocked"; return; }
...
function getPacket(project: StoredProjectRecord, packetId: string) { return getPackets(project).find((packet: any) => packet.packetId === packetId) || null; }
function getNextPendingPacket(project: StoredProjectRecord) { return getPackets(project).find((packet: any) => packet.status === "pending") || null; }
function getRepairablePackets(project: StoredProjectRecord) {
  ...
  return packets.filter((packet: any) => packet.status === "blocked" && shouldRetry(gitResults[`${packet.packetId}:commit_files`]?.status));
}
...
function setGitOperationResult(project: StoredProjectRecord, result: GitOperationResult) { project.gitResults = { ...((project.gitResults || {}) as Record<string, GitOperationResult>), [result.operationId]: result }; }
function getGitOperationResult(project: StoredProjectRecord, operationId: string) { return ((project.gitResults || {}) as Record<string, GitOperationResult>)[operationId] || null; }
...
function validateChangedFiles(changedFiles: ChangedFile[]) {
  return changedFiles.map((file, index) => {
    if (!file?.path || typeof file.body !== "string") throw new Error(`Executor returned invalid changed file payload: ${JSON.stringify({ index, path: file?.path ?? null, hasBody: typeof file?.body === "string" })}`);
    return { path: file.path, content: file.body };
  });
}
async function persistProject(config: RuntimeConfig, project: StoredProjectRecord) { project.updatedAt = now(); await config.repository.repo.upsertProject(project); }

function recordGitFailure(project: StoredProjectRecord, operationId: string, packetId: string, branchName: string | undefined, message: string) {
  const kind = classifyGitFailure(message);
  const recoverable = isRecoverableGitFailure(kind);
  setGitOperationStatus(project, operationId, recoverable ? "succeeded" : "failed");
  setGitOperationResult(project, { operationId, status: recoverable ? "succeeded" : "failed", branchName, error: message, updatedAt: now() });
  const runs = { ...((project.runs || {}) as Record<string, any>) };
  const current = runs[packetId] || { projectId: project.projectId, packetId, status: "queued", retryCount: 0, logs: [] };
  runs[packetId] = { ...current, logs: [...(current.logs || []), { timestamp: now(), level: recoverable ? "warn" : "error", event: "git_operation_result", details: { operationId, kind, recoverable, message } }] };
  project.runs = runs;
  return recoverable;
}
...
async function runGitHubLifecycle(config: RuntimeConfig, project: StoredProjectRecord, packet: any, changedFiles: ChangedFile[]) {
  ...
  const existingBranchResult = getGitOperationResult(project, branchOp.operationId);
  await dedupeOperation(existingBranchResult?.status === "succeeded" ? existingBranchResult : null, async () => {
    ...
    try {
      ...
      const result = { operationId: branchOp.operationId, status: "succeeded", branchName: packet.branchName, updatedAt: now() } as GitOperationResult;
      setGitOperationStatus(project, branchOp.operationId, "succeeded");
      setGitOperationResult(project, result);
      await persistProject(config, project);
      return result;
    } catch (error: any) {
      const normalized = normalizeGitHubError(error);
      const reconciled = reconcileBranch(existingBranchResult, normalized);
      if (reconciled.status === "succeeded") {
        setGitOperationStatus(project, branchOp.operationId, "succeeded");
        setGitOperationResult(project, { ...reconciled, operationId: branchOp.operationId, branchName: packet.branchName });
        await persistProject(config, project);
        return { ...reconciled, operationId: branchOp.operationId, branchName: packet.branchName };
      }
      recordGitFailure(project, branchOp.operationId, packet.packetId, packet.branchName, normalized);
      await persistProject(config, project);
      throw error;
    }
  });
  ...
  const existingCommitResult = getGitOperationResult(project, commitOp.operationId);
  await dedupeOperation(existingCommitResult?.status === "succeeded" ? existingCommitResult : null, async () => {
    ...
    try {
      ...
      const result = { operationId: commitOp.operationId, status: "succeeded", branchName: packet.branchName, commitSha: commit.sha, updatedAt: now() } as GitOperationResult;
      ...
      const normalized = normalizeGitHubError(error);
      recordGitFailure(project, commitOp.operationId, packet.packetId, packet.branchName, normalized);
      await persistProject(config, project);
      throw error;
    }
  });
  ...
  const existingPrResult = getGitOperationResult(project, prOp.operationId);
  await dedupeOperation(existingPrResult?.status === "succeeded" ? existingPrResult : null, async () => {
    ...
    try {
      ...
      const result = { operationId: prOp.operationId, status: "succeeded", branchName: packet.branchName, prUrl: pr.html_url, updatedAt: now() } as GitOperationResult;
      ...
      const normalized = normalizeGitHubError(error);
      const reconciled = reconcilePullRequest(existingPrResult, normalized);
      if (reconciled.status === "succeeded") {
        setGitOperationStatus(project, prOp.operationId, "succeeded");
        setGitOperationResult(project, { ...reconciled, operationId: prOp.operationId, branchName: packet.branchName });
        await persistProject(config, project);
        return { ...reconciled, operationId: prOp.operationId, branchName: packet.branchName };
      }
      recordGitFailure(project, prOp.operationId, packet.packetId, packet.branchName, normalized);
      await persistProject(config, project);
      throw error;
    }
  });
...
async function processJob(config: RuntimeConfig, job: QueueJobRecord) {
  ...
  if (!project) { await finalizeJob(job.job_id, "failed", `Project not found: ${job.project_id}`); return; }
  const packet = getPacket(project, job.packet_id);
  if (!packet) { await finalizeJob(job.job_id, "failed", `Packet not found: ${job.packet_id}`); return; }
  ...
  try {
    ...
    const validation = await runValidation(project.projectId, packet.packetId);
    ...
    if (validation.status !== "passed") {
      ...
      recomputeProjectStatus(project);
      await persistProject(config, project);
      await finalizeJob(job.job_id, "failed", validation.summary || "Validation failed");
      return;
    }
...
export function buildApp(config: RuntimeConfig) {
  ...
  app.post("/api/projects/intake", async (req, res) => {
    ...
    } catch (error) { return handleRouteError(res, config, error, "POST /api/projects/intake", actor); }
  });
  app.post("/api/projects/:projectId/compile", async (req, res) => {
    ...
    } catch (error) { return handleRouteError(res, config, error, "POST /api/projects/:projectId/compile", actor); }
  });
  app.post("/api/projects/:projectId/plan", async (req, res) => {
    ...
    try {
      ...
      await repo.upsertProject(updated);
      return res.json({ projectId: updated.projectId, status: updated.status, actorId: actor.actorId });
    } catch (error) { return handleRouteError(res, config, error, "POST /api/projects/:projectId/plan", actor); }
  });
  app.post("/api/projects/:projectId/git/result", async (req, res) => {
    ...
    try {
      ...
      const updated: StoredProjectRecord = { ...project, gitResults: { ...(project.gitResults || {}), [result.operationId || `result_${Date.now()}`]: result }, updatedAt: now() };
      await repo.upsertProject(updated);
      return res.json({ status: "recorded", actorId: actor.actorId });
    } catch (error) { return handleRouteError(res, config, error, "POST /api/projects/:projectId/git/result", actor); }
  });
  app.post("/api/projects/:projectId/dispatch/execute-next", async (req, res) => {
    ...
    } catch (error) { return handleRouteError(res, config, error, "POST /api/projects/:projectId/dispatch/execute-next", actor); }
  });
  app.post("/api/projects/:projectId/repair/replay", async (req, res) => {
    ...
    } catch (error) { return handleRouteError(res, config, error, "POST /api/projects/:projectId/repair/replay", actor); }
  });
  app.get("/api/projects/:projectId/status", async (req, res) => {
    ...
    } catch (error) { return handleRouteError(res, config, error, "GET /api/projects/:projectId/status", actor); }
  });
  return app;
}
