...
export function LiveUIBuilderSourceSyncPanel({ sourceSyncStatus, sourceSyncResult, onDryRun, onApply, canApply, hasRealFileAdapter = false, writeSafetyStatus = "writes disabled", explicitConfirmed = false, onExplicitConfirmChange, lastSourceApplyProof, rollbackAvailable = false, rollbackStatus = "Rollback requires server-side local adapter.", sourceRollbackDryRun }: SourceSyncPanelProps) {
  ...
  return (<section className="vibe-rail-card" aria-label="Live UI source sync panel">
      ...
      {!!sourceSyncResult?.patchSummary?.scalabilityPerformance && <><p>Scalability plan id: {sourceSyncResult.patchSummary.scalabilityPerformance.scalabilityPlanId}</p>
      <p>Scalability document node count: {sourceSyncResult.patchSummary.scalabilityPerformance.documentNodeCount}</p>
      <p>Scalability source file count: {sourceSyncResult.patchSummary.scalabilityPerformance.sourceFileCount}</p>
      <p>Scalability total source bytes: {sourceSyncResult.patchSummary.scalabilityPerformance.totalSourceBytes}</p>
      <p>Scalability operation count: {sourceSyncResult.patchSummary.scalabilityPerformance.operationCount}</p>
      <p>Scalability generated file count: {sourceSyncResult.patchSummary.scalabilityPerformance.generatedFileCount}</p>
      <p>Scalability identity count: {sourceSyncResult.patchSummary.scalabilityPerformance.identityCount}</p>
      <p>Scalability dependency count: {sourceSyncResult.patchSummary.scalabilityPerformance.dependencyCount}</p>
      <p>Scalability risk level: {sourceSyncResult.patchSummary.scalabilityPerformance.riskLevel}</p>
      <p>Scalability manual review required: {String(sourceSyncResult.patchSummary.scalabilityPerformance.requiresManualReview)}</p>
      <p>Scalability chunk count: {sourceSyncResult.patchSummary.scalabilityPerformance.recommendedChunkCount}</p>
      <p>Scalability index recommendation count: {sourceSyncResult.patchSummary.scalabilityPerformance.recommendedIndexCount}</p>
      <p>Scalability pagination recommendation count: {sourceSyncResult.patchSummary.scalabilityPerformance.recommendedPaginationCount}</p>
      <p>Scalability blocked reason count: {sourceSyncResult.patchSummary.scalabilityPerformance.blockedReasonCount}</p>
