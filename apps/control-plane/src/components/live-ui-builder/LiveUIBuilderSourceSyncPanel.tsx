...
type SourceSyncPanelProps = {
  ...
  sourceSyncResult?: { patchSummary?: { changedFiles: string[]; operationCount: number; manualReviewCount: number; confidenceCounts?: Record<string, number>; sourceKinds?: string[]; routeFilesToCreate?: string[]; manualReviewReasons?: string[]; identitySummary?: { coverageCount: number; high: number; medium: number; low: number; stale: number; manualReviewRequired: number; selectedNodeIdentitySummary?: string }; multiFilePlanId?: string; dependencyCount?: number; riskLevel?: "low"|"medium"|"high"; requiresManualReview?: boolean; orderedFiles?: string[]; uxControlPlan?: { uxControlPlanId: string; activeMode: string; selectedNodeId?: string; controls: { enabled: boolean }[]; pendingState: { label: string }; debouncePlan: { recommendations: string[] }; undoRedoState: { undoEnabled: boolean; redoEnabled: boolean }; recoveryMessages: { message: string }[]; caveat: string }; fullProjectGeneration?: { planId: string; normalizedProjectSlug: string; framework: "next"|"vite-react"|"node-api"|"unknown"; generatedFileCount: number; directoryCount: number; conflictCount: number; normalizationIssueCount: number; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; orderedFilePathsPreview: string[]; caveat: string }; stylePlan?: { stylePlanId: string; tokenCount: number; categoriesPresent: string[]; outputMode: "cssVariables"|"tailwindTheme"|"inlinePreview"|"unknown"; targetFilePath?: string; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; issueCount: number; cssVariableNames: string[]; caveat: string }; dataStateApiWiring?: { wiringPlanId: string; dataBindingCount: number; stateBindingCount: number; stateActionCount: number; apiEndpointCount: number; apiRequestBindingCount: number; affectedNodeCount: number; affectedFileCount: number; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; blockedReasonCount: number; orderedOperationLabels: string[]; caveat: string }; reliabilityRepair?: { repairPlanId: string; failureCount: number; selectedStrategyCount: number; attemptCount: number; maxAttempts: number; nextAttemptIndex: number; rollbackRequired: boolean; rollbackProofRequired: boolean; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; blockedReasonCount: number; strategyLabels: string[]; caveat: string }; scalabilityPerformance?: { scalabilityPlanId: string; changedFileCount: number; operationCount: number; chunkCount: number; averageChunkSize: number; maxChunkSize: number; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; caveat: string }; exportDeployPlan?: { exportDeployPlanId: string; provider: string; framework: string; targetName: string; bundleFileCount: number; bundleTotalBytes: number; readinessGateCount: number; failedGateCount: number; rollbackPlanRequired: boolean; previewUrlPlanned: boolean; liveDeployBlocked: boolean; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; blockedReasonCount: number; affectedFilePaths: string[]; caveat: string }; platformBuilderPlan?: { platformBuilderPlanId: string; targetPlatform: string; builderKind: string; framework: string; normalizedProjectSlug: string; sourceFileCount: number; sourceTotalBytes: number; requiredAssetCount: number; missingAssetCount: number; manifestRequirementCount: number; failedCompatibilityGateCount: number; publishBlocked: boolean; buildExecutionBlocked: boolean; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; blockedReasonCount: number; affectedFilePaths: string[]; caveat: string }; orchestration?: { orchestrationPlanId: string; stageCount: number; dependencyCount: number; gateCount: number; riskLevel: "low"|"medium"|"high"; requiresManualReview: boolean; readyForSourceApply: boolean; readyForExportPlanning: boolean; readyForPlatformPlanning: boolean; executionBlocked: boolean; blockedReasonCount: number; changedFileCount: number; affectedFileCount: number; orderedStageIdsPreview: string[]; caveat: string } }; blockedReasons?: string[]; caveat?: string };
  onDryRun: () => void;
  onApply: () => void;
  canApply: boolean;
...
export function LiveUIBuilderSourceSyncPanel({ sourceSyncStatus, sourceSyncResult, onDryRun, onApply, canApply, hasRealFileAdapter = false, writeSafetyStatus = "writes disabled", explicitConfirmed = false, onExplicitConfirmChange, lastSourceApplyProof, rollbackAvailable = false, rollbackStatus = "Rollback requires server-side local adapter.", sourceRollbackDryRun }: SourceSyncPanelProps) {
  ...
  return (<section className="vibe-rail-card" aria-label="Live UI source sync panel">
      ...
      <p>UX selected node id: {uxControlPlan.selectedNodeId ?? "none"}</p>
      <p>UX controls enabled: {enabledControlCount}</p>
      <p>UX controls disabled: {disabledControlCount}</p>
      <p>UX pending label: {uxControlPlan.pendingState.label}</p>
      <p>UX debounce recommendations: {uxControlPlan.debouncePlan.recommendations.slice(0,5).join(", ") || "none"}</p>
      <p>UX undo enabled: {String(uxControlPlan.undoRedoState.undoEnabled)}</p>
      <p>UX redo enabled: {String(uxControlPlan.undoRedoState.redoEnabled)}</p>
      <p>UX recovery message count: {uxControlPlan.recoveryMessages.length}</p>
      <p>UX recovery messages preview: {uxControlPlan.recoveryMessages.slice(0,5).map((m) => m.message).join(" | ") || "none"}</p>
      ...
      {!!sourceSyncResult?.patchSummary?.exportDeployPlan && <><p>Export deploy plan id: {sourceSyncResult.patchSummary.exportDeployPlan.exportDeployPlanId}</p>
      <p>Export provider: {sourceSyncResult.patchSummary.exportDeployPlan.provider}</p>
      <p>Export framework: {sourceSyncResult.patchSummary.exportDeployPlan.framework}</p>
      <p>Export target name: {sourceSyncResult.patchSummary.exportDeployPlan.targetName}</p>
      <p>Bundle file count: {sourceSyncResult.patchSummary.exportDeployPlan.bundleFileCount}</p>
      <p>Bundle total bytes: {sourceSyncResult.patchSummary.exportDeployPlan.bundleTotalBytes}</p>
      <p>Readiness gate count: {sourceSyncResult.patchSummary.exportDeployPlan.readinessGateCount}</p>
      <p>Failed gate count: {sourceSyncResult.patchSummary.exportDeployPlan.failedGateCount}</p>
      <p>Rollback plan required: {String(sourceSyncResult.patchSummary.exportDeployPlan.rollbackPlanRequired)}</p>
      <p>Preview URL planned: {String(sourceSyncResult.patchSummary.exportDeployPlan.previewUrlPlanned)}</p>
      <p>Live deploy blocked: {String(sourceSyncResult.patchSummary.exportDeployPlan.liveDeployBlocked)}</p>
      <p>Export risk level: {sourceSyncResult.patchSummary.exportDeployPlan.riskLevel}</p>
      <p>Export manual review required: {String(sourceSyncResult.patchSummary.exportDeployPlan.requiresManualReview)}</p>
      <p>Export blocked reason count: {sourceSyncResult.patchSummary.exportDeployPlan.blockedReasonCount}</p>
      <p>Export affected files preview: {sourceSyncResult.patchSummary.exportDeployPlan.affectedFilePaths.slice(0,10).join(", ") || "none"}</p>
      ...
      {!!sourceSyncResult?.patchSummary?.platformBuilderPlan && <><p>Platform builder plan id: {sourceSyncResult.patchSummary.platformBuilderPlan.platformBuilderPlanId}</p>
      <p>Platform target: {sourceSyncResult.patchSummary.platformBuilderPlan.targetPlatform}</p>
      <p>Platform builder kind: {sourceSyncResult.patchSummary.platformBuilderPlan.builderKind}</p>
      <p>Platform framework: {sourceSyncResult.patchSummary.platformBuilderPlan.framework}</p>
      <p>Platform project slug: {sourceSyncResult.patchSummary.platformBuilderPlan.normalizedProjectSlug}</p>
      <p>Platform source file count: {sourceSyncResult.patchSummary.platformBuilderPlan.sourceFileCount}</p>
      <p>Platform source total bytes: {sourceSyncResult.patchSummary.platformBuilderPlan.sourceTotalBytes}</p>
      <p>Platform required asset count: {sourceSyncResult.patchSummary.platformBuilderPlan.requiredAssetCount}</p>
      <p>Platform missing asset count: {sourceSyncResult.patchSummary.platformBuilderPlan.missingAssetCount}</p>
      <p>Platform manifest requirement count: {sourceSyncResult.patchSummary.platformBuilderPlan.manifestRequirementCount}</p>
      <p>Platform failed compatibility gate count: {sourceSyncResult.patchSummary.platformBuilderPlan.failedCompatibilityGateCount}</p>
      <p>Platform publish blocked: {String(sourceSyncResult.patchSummary.platformBuilderPlan.publishBlocked)}</p>
      <p>Platform build execution blocked: {String(sourceSyncResult.patchSummary.platformBuilderPlan.buildExecutionBlocked)}</p>
      <p>Platform risk level: {sourceSyncResult.patchSummary.platformBuilderPlan.riskLevel}</p>
      <p>Platform manual review required: {String(sourceSyncResult.patchSummary.platformBuilderPlan.requiresManualReview)}</p>
      <p>Platform blocked reason count: {sourceSyncResult.patchSummary.platformBuilderPlan.blockedReasonCount}</p>
      <p>Platform affected files preview: {sourceSyncResult.patchSummary.platformBuilderPlan.affectedFilePaths.slice(0,10).join(", ") || "none"}</p>
      ...
      {!!orchestration && <><p>Orchestration plan id: {orchestration.orchestrationPlanId}</p>
      <p>Orchestration stage count: {orchestration.stageCount}</p>
      <p>Orchestration dependency count: {orchestration.dependencyCount}</p>
      <p>Orchestration gate count: {orchestration.gateCount}</p>
      <p>Orchestration risk level: {orchestration.riskLevel}</p>
      <p>Orchestration manual review required: {String(orchestration.requiresManualReview)}</p>
      <p>Orchestration readyForSourceApply: {String(orchestration.readyForSourceApply)}</p>
      <p>Orchestration readyForExportPlanning: {String(orchestration.readyForExportPlanning)}</p>
      <p>Orchestration readyForPlatformPlanning: {String(orchestration.readyForPlatformPlanning)}</p>
      <p>Orchestration executionBlocked: {String(orchestration.executionBlocked)}</p>
      <p>Orchestration blocked reason count: {orchestration.blockedReasonCount}</p>
      <p>Orchestration changed file count: {orchestration.changedFileCount}</p>
      <p>Orchestration affected file count: {orchestration.affectedFileCount}</p>
      <p>Orchestration ordered stages preview: {orchestration.orderedStageIdsPreview.slice(0,15).join(", ") || "none"}</p>
      <p>Live Builder orchestration is dry-run only and does not write files, build, package, deploy, publish, call provider/platform APIs, open emulators, or prove runtime correctness.</p></>}
