  ...
  return (
    <section className="pro-dashboard" aria-label="Pro dashboard" data-project-id={projectId}>
      <aside className="pro-dashboard-sidebar" aria-label="Botomatic sidebar">
        ...
        <div className="pro-sidebar-card"><h3>Recent Projects</h3>{proRecentProjects.length > 0 ? proRecentProjects.map((recentProject, index) => <div className={`pro-sidebar-row${index === 0 ? " is-active" : ""}`} key={recentProject.name}><span>{recentProject.name}</span><small>{recentProject.updated}</small></div>) : <small>No projects yet</small>}<button type="button" className="pro-link-button">View all projects →</button></div>
        <div className="pro-sidebar-upgrade"><h3>Account</h3><button type="button">Upgrade options</button></div>
      </aside>
