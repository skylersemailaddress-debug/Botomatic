...
export function getFirstRunFallback(projectId: string): FirstRunState {
  ...
  return {
    ...
    hasObjective: false,
    hasNextStep: false,
    hasOrchestrationRun: false,
    hasExecutionRun: false,
    hasRuntimePreview: false,
    canLaunch: false,
    steps,
    primaryAction: steps[0],
    message: "No first-run state yet",
  };
}
...
export function deriveFirstRunState(input: DeriveInput): FirstRunState {
  ...
  const canLaunch = hasRuntimePreview && !executionBlocked;
  ...
  const steps: FirstRunStep[] = [
    ...
    { id: "runtime", label: canLaunch ? "Launch" : "Review launch requirements", status: canLaunch ? "available" : "blocked", detail: canLaunch ? "Runtime preview available" : "Launch proof missing", disabled: !canLaunch },
  ];
  ...
  return {
    ...
    hasProjectState: Boolean(input.projectState || input.resume),
    hasObjective,
    hasNextStep,
    hasOrchestrationRun,
    hasExecutionRun,
...
export async function getFirstRunState(projectId: string): Promise<ApiResult<FirstRunState>> {
  const [resumeResult, stateResult, executionResult, runtime] = await Promise.all([
    getProjectResume(projectId),
    getProjectState(projectId),
    getExecutionRun(projectId),
    getProjectRuntimeState(projectId),
  ]);

  if (!resumeResult.ok && !stateResult.ok && !executionResult.ok && !runtime.status && !runtime.previewUrl) {
    return { ok: false, state: "empty", status: 404, message: "No first-run state yet" };
  }

  return {
    ok: true,
    data: deriveFirstRunState({
      projectId,
      resume: resumeResult.ok ? resumeResult.data : undefined,
      projectState: stateResult.ok ? stateResult.data : undefined,
      execution: executionResult.ok ? executionResult.data : undefined,
      runtime,
    }),
  };
}
