...
import { applySecurityHeaders, enforceLoginRateLimit, blockCrossSiteMutation } from "../../../../server/security";
...
export async function POST(req: NextRequest) {
  ...
  const rateLimited = enforceLoginRateLimit(req);
  ...
  if (!secret) {
    return applySecurityHeaders(NextResponse.json({ error: "UI password not configured — set BOTOMATIC_UI_PASSWORD" }, { status: 503 }));
  }
  ...
  if (!match) {
    return applySecurityHeaders(NextResponse.json({ error: "Invalid password" }, { status: 401 }));
  }
