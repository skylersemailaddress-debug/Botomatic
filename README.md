...
- Large-file intake posture: default upload limit is 250 MB (`BOTOMATIC_MAX_UPLOAD_MB`), extracted archive limit is 2000 MB (`BOTOMATIC_MAX_EXTRACTED_MB`), and zip file-count limit is 20000 (`BOTOMATIC_MAX_ZIP_FILES`), with disk-backed upload handling and archive safety checks.
- Multi-source intake posture: Intake Hub supports GitHub URLs (repo/branch/PR), cloud storage links, pasted text, local folder manifests, browser uploads (zip/file/pdf), and existing project references through a unified source registry with per-source routing decisions.
- Multi-source intake safety posture: intake registration/fetch path emits lifecycle events (`intake_source_registered` through `intake_manifest_written`/`intake_completed`), writes per-source intake manifests, enforces auth-gated metadata-only fallback for private or connector-bound sources, and declares no-code-execution intake behavior.
- Archive safety posture: zip intake rejects traversal/absolute paths and symlink escapes, enforces extracted size and file-count ceilings, skips heavy build/vendor folders by default, and summarizes binary-only files while extracting text/code/spec files.
- Intake UX posture: upload/extraction progress states are surfaced during ingestion (`upload_started` through `ingestion_completed`/`ingestion_failed`) with clear error messages for oversize uploads, unsupported types, unsafe archive paths, zip bomb conditions, and extraction limit breaches.
- Intake scale caveat: for very large repositories, GitHub/cloud-link intake is still preferred when available to avoid oversized browser uploads.
